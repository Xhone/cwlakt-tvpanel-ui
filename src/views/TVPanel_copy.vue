...
export default defineComponent({
  ...
  setup() {
    ...
    //const Macid=ref(route.query.Macid as string || localStorage.getItem('Macid') || '01');
    const Macid = ref(getQueryParam('macid') || localStorage.getItem('Macid') || '01');
    const clearTimer = () => {
      if (timer.value) {
    ...
    const formatMacid = (macid: string): string => {
      if (!macid) return 'Line'+'1'; // Default value if empty

      // Remove all leading zeros and ensure at least "1"
      const formatted = macid.replace(/^0+/, '') || '1';
    
      // Validate the result
      if (!/^\d+$/.test(formatted)) {
        console.warn(`Invalid Macid format: ${macid}`);
        return '1';
      }
    ...
    const fetchData = async (forceRefresh = false) => {
      ...
      try {
        const response = await request.get<ProductionData>('/TV/GetDailyProduction', { 
          Macid: Macid.value,
          timestamp: Date.now(),
          forceRefresh // 传递给 aktApi 是否强制刷新缓存
        });
    ...
    const formatTimeRange = (hour: number): string => {
      const start = hour + 6;
      const end = start + 1;
      const startTime = start.toString().padStart(2, '0');
      const endTime = end.toString().padStart(2, '0');
  
      // Special case for the last column (16:00)
      if (hour === 10) {
        return `${startTime}:00 After`;
      }
